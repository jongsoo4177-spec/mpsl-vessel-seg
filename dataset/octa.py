"""dataset_octa.py — OCTA 망막 혈관 분할 데이터셋."""
...
class OCTADataset(Dataset):
    """
    OCTA(Optical Coherence Tomography Angiography) 망막 혈관 분할 데이터셋.
    ...
    def __getitem__(self, index):
        ...
        if modify_mask.max() > 100:           # [0,255] 범위면 정규화
            modify_mask /= 255.0
...
class Test_OCTADataset(Dataset):
    """Test dataset for OCTA retinal vessel segmentation."""
    ...
    def __getitem__(self, index):
        ...
        # Compute crop bounds clamped to image extents
        ...
        # Remap click point to crop-local coordinates, then scale to model input size
