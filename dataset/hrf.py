"""dataset_hrf.py — HRF 망막 혈관 분할 데이터셋."""
...
class HRFDataset(Dataset):
    """
    HRF(High-Resolution Fundus) 망막 혈관 분할 데이터셋.

    CHASEDataset과 동일한 augmentation 로직을 사용하나 다음 두 가지가 다르다:
        - 크롭 크기: 512×512 (CHASE는 256×256)
        - 디렉토리 이름: images/mask (CHASE는 Image/Mask)

    텍스트 가이드 augmentation:
        ...
        range_num 0  포인트 수준  단일 세그먼트 선택
        range_num 1  로컬         연속 세그먼트 범위 선택
        range_num 2  전역         전체 마스크 사용 (case 0 한정)
    ...
    def __getitem__(self, index):
        ...
        # ── 크롭 전략 ────────────────────────────────────────────────────────
        # case 1: gt에만 있는 영역에 클릭 후 해당 좌표 중심으로 crop_with_padding
        # 나머지: transforms.RandomCrop
        if case_num == 1:
...
class Test_HRFDataset(Dataset):
    """Test dataset for HRF retinal vessel segmentation with optic-disc masking."""
    ...
    def __init__(self, args, dataset_path, split='train',
        ...
        self.prompt = prompt
        self.img_size = args.image_size
        self.out_size = (args.out_size, args.out_size)
        self.threshold = 0.49
    ...
    def __getitem__(self, index):
        ...
        pt = np.array([self.out_size / 2, self.out_size / 2], dtype=np.float32)
