...
from .modeling import ImageEncoderViT,  PromptEncoder, Sam, Sam_TPP, Ours, TextEncoder ,MaskDecoderHQ, TextPredictor, MaskDecoderHQ_PSM
...
def _build_sam(
    ...
    if args.train_stage == 'train_backbone':
        sam = Sam(
            ...
            hq_decoder= MaskDecoderHQ(args.model_type, args.decoder_path),
            pixel_mean=[123.675, 116.28, 103.53],
            pixel_std=[58.395, 57.12, 57.375],
        )
    
    elif args.train_stage == 'train_tpp':
        sam = Sam_TPP(
            args,
            image_encoder=ImageEncoderViT(
                args = args,
            ...
            text_predictor= TextPredictor(args.model_type, args.decoder_path, args.num_cls),
            pixel_mean=[123.675, 116.28, 103.53],
            pixel_std=[58.395, 57.12, 57.375],
        )

    elif args.train_stage == 'train_psm' or args.eval == True:
        sam = Ours(
            args,
            image_encoder=ImageEncoderViT(
                args = args,
            ...
            text_encoder=TextEncoder(args.clip_ckpt),
            hq_decoder= MaskDecoderHQ_PSM(args.model_type, args.decoder_path),
            text_predictor= TextPredictor(args.model_type, args.decoder_path, args.num_cls),
            pixel_mean=[123.675, 116.28, 103.53],
            pixel_std=[58.395, 57.12, 57.375],
        )
