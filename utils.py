...
def cka_loss(gram_featureA, gram_featureB):
    """Compute Centered Kernel Alignment (CKA) similarity between two Gram matrices."""
...
def visualization(
    imgs, pred_masks, gt_masks, prev_masks, save_path,
    mode=None,       # 'backbone' | 'psm' | 'tpp'
    reverse=False,
    points=None,
    pt_labels=None,
    text=None,
    ...
    # ── branch by channel count (c==2 / c>2) ──────────
    if c == 2:
    ...
    if c > 2:
        preds = [pred_masks[:,i].unsqueeze(1).expand(b,3,h,w) for i in range(c)]
        gts   = [gt_masks[:,i].unsqueeze(1).expand(b,3,h,w)   for i in range(c)]
        compose = torch.cat([imgs[:row_num]] + preds + gts, 0)
        vutils.save_image(compose, fp=save_path, nrow=row_num, padding=10)
        return

    # ── single-channel (c==1) path ────────────────────
    imgs       = torchvision.transforms.Resize((h,w))(imgs)
    prev_masks = torch.zeros_like(pred_masks) if prev_masks is None else \
                 torchvision.transforms.Resize((h,w))(prev_masks)
    ...
    prev_masks = torch.stack([
        prev_masks[:,0].clone(),
        prev_masks[:,1].clone(),
        prev_masks[:,2].clone(),
    ], dim=1)

    # ── draw click points onto prev_masks ─────────────
    if points is not None:
        p = np.round(points.cpu() / args.image_size * h).to(dtype=torch.int)
        for i in range(b):
            for j in range(len(points)):   # iterate over all click points
                r, g_ch = (1, 0) if pt_labels[j] == 0 else (0, 1)
                prev_masks[i, 0, p[j,0]-3:p[j,0]+3, p[j,1]-3:p[j,1]+3] = float(r)
                prev_masks[i, 1, p[j,0]-3:p[j,0]+3, p[j,1]-3:p[j,1]+3] = float(g_ch)
                prev_masks[i, 2, p[j,0]-3:p[j,0]+3, p[j,1]-3:p[j,1]+3] = 0.0


    # ── add text labels to each panel ─────────────────
    if mode == 'tpp' and pred_text is not None:
        imgs       = add_tag(imgs,       'Input image')
        prev_masks = add_tag(prev_masks, 'Prev, GT Prompt: '   + gt_text[0])
        pred_masks = add_tag(pred_masks, 'Pred Prompt: ' + pred_text[0])
        gt_masks   = add_tag(gt_masks,   'GT Mask')
    elif text is not None:
        imgs       = add_tag(imgs,       'Input image')
        prev_masks = add_tag(prev_masks, ('Prev, Syns Prompt: ' + text[0])
                             if mode == 'psm' else ('Prev, Prompt: ' + text[0]))
        pred_masks = add_tag(pred_masks, 'Prediction mask')
        gt_masks   = add_tag(gt_masks,   'GT')

    # ── save the composed image grid ──────────────────
    tup_list = [
        torch.cat((imgs[i].unsqueeze(0), prev_masks[i].unsqueeze(0),
                    pred_masks[i].unsqueeze(0), gt_masks[i].unsqueeze(0)), 0)
        for i in range(b)
    ]
...
def eval_cl(pred,true_mask_p,threshold):
        cl = 0
        th = threshold
        ...
        '''iou for numpy'''
        cl = clDice(disc_pred[0],disc_mask[0])
...
def add_tag(image, tag='nodefined', tag_h=40):
    """Append a white text banner below a (1, C, H, W) torch tensor and return it."""
    if image.requires_grad:
        image = image[0].permute(1, 2, 0).detach().cpu().numpy().astype(np.float32)  # convert PyTorch tensor to NumPy array
    else:
    ...
    image = torch.tensor(image).permute(2, 0, 1).float()  # convert back to PyTorch tensor
    return image.unsqueeze(0)

def add_tag_1c_np(image, tag='nodefined', tag_h=40):
    """Append a text banner below a single-channel (H x W) NumPy image."""
...
def add_tag_3c_np(image, tag='nodefined', tag_h=40):
    """Append a text banner below a 3-channel (H x W x 3) NumPy image."""
    image = image.astype(np.float32)
    H, W = image.shape[0], image.shape[1]
...
def random_crop(image: np.ndarray, crop_size=(256, 256)):
    """Return a randomly cropped region of the given size from image."""
    h, w = image.shape[:2]  # get image height and width
    ch, cw = crop_size  # crop dimensions

    if h < ch or w < cw:
        raise ValueError("Crop size exceeds the original image dimensions.")
...
# example usage
image = np.random.rand(512, 512, 3)  # 512x512 RGB image
...
def get_next_click(gt_mask, pred_mask, points):
    """Select the next corrective click based on the largest error region.

    Computes false-negative (FN) and false-positive (FP) masks, finds the
    largest connected component in each, and places a positive click on the
    largest FN region or a negative click on the largest FP region.

    Args:
        gt_mask:   Ground-truth binary mask (H x W numpy array).
        pred_mask: Predicted binary mask (H x W numpy array).
        points:    Current prompt tuple (coords_tensor, labels_tensor).

    Returns:
        (coords, labels): Updated point tensors with the new click appended.
    """
    ...
    coords = points[0].clone()
    labels = points[1].clone()

    # Find largest false-negative connected component
    fn_num_labels, fn_labels, fn_stats, fn_centroids = cv2.connectedComponentsWithStats(fn_mask)
    if fn_num_labels > 1:
        fn_areas = fn_stats[1:, cv2.CC_STAT_AREA]
        largest_fn_area_idx = np.argmax(fn_areas) + 1
        fn_max = np.max(fn_areas)
        fn_coord = fn_centroids[largest_fn_area_idx]  # (x, y) 

    # Find largest false-positive connected component
    fp_num_labels, fp_labels, fp_stats, fp_centroids = cv2.connectedComponentsWithStats(fp_mask)
    if fp_num_labels > 1:
        fp_areas = fp_stats[1:, cv2.CC_STAT_AREA]
        largest_fp_area_idx = np.argmax(fp_areas) + 1
        fp_max = np.max(fp_areas)
        fp_coord = fp_centroids[largest_fp_area_idx]  # (x, y) 

    is_positive = fn_max > fp_max

    if is_positive and fn_coord is not None:
        coords_x, coords_y = fn_coord[0], fn_coord[1]  # centroid -> (x, y)
        coords[0, 0, 0] = float(coords_y) / args.out_size * args.image_size
        coords[0, 0, 1] = float(coords_x) / args.out_size * args.image_size
        labels[0, 0] = 1
    elif not is_positive and fp_coord is not None:
        coords_x, coords_y = fp_coord[0], fp_coord[1]
        coords[0, 0, 0] = float(coords_y) / args.out_size * args.image_size
        coords[0, 0, 1] = float(coords_x) / args.out_size * args.image_size
        labels[0, 0] = 0
    else:
        # Fallback: both FN and FP are empty; return origin with negative label
        coords[0, 0, 0] = 0
        coords[0, 0, 1] = 0
        labels[0, 0] = 0
...
def return_text(num):
    """Map a predicted class index to its edit command string."""
    category = ['Make thinner', 'Make thicker', 'Extend', 'Remove', 'Make a connection']
    return [category[int(num)]]
